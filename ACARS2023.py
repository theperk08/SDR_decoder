    ...
    while fini==False:    
        ...
        if not(result=="rien"):
            if time.time()-dernierheure>0.9: #pour éviter de prendre plusieurs fois le même signal
                ...
                #    write(direc+str(datetime.today())[:10]+"_enreg_analyse_ACARS_"+str(nfich)+"f.wav",RATE,chainefic)
...
def lance2a(stream2,sample_c,rb2):
    ...
    try: 
     ...
     if rb2.GetSelection()==0:
         ...
         write(direc+str(datetime.today())[:10]+"_enreg_analyse_ACARS_"+str(nfich)+"g.wav",RATE,chainefic)
    except IOError: 
     print("Error Recording")
...
def lancetest1(stream0,heure):
    ...
    """
    x = np.arange(Tdebut,Tfin, Pas)   # start,stop,step
    z = Amplitude*np.cos(FREQ*2*np.pi*(x-Decalage)/CHANNELS)    
    
    plt.plot(x,z, '--', color='blue', label="sinusoide")
    plt.plot(x,signal0, '-', color='orange', label="signal réel")
    plt.xlabel('temps')
    plt.ylabel('z et signal')
    plt.legend()
    plt.show()    
    
    Empiriquement :
    signal-cos > seuil (15000) = 0
    signla-cos < seuil (15000) = 1
    """
    ...
    if affiche:
        ...
        date1=date.today()
        fichier=open(direc+"ACARS_signal_"+str(date1)+".html","a")
